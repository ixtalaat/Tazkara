...
export class AdminDashboardComponent implements OnInit {
  ...
  data = signal<AdminOverview | null>(null); loading = signal(true); error = signal(''); activeTab = signal<'review' | 'users' | 'categories'>('review'); actionId = signal<string | null>(null); categoryName = signal('');
  ...
  addCategory(): void { const name = this.categoryName().trim(); if (!name) return; this.service.createCategory(name).subscribe({ next: r => { if (r.success) { this.categoryName.set(''); this.load(); } else this.error.set(r.message); }, error: e => this.error.set(e.error?.message || 'Unable to create category.') }); }
